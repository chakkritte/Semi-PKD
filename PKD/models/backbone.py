...
import kornia
import darmo
from PKD.models.head import Decoder
import timm
...
class ResNestModel(nn.Module):
    ...
    def __init__(self, num_channels=3, train_enc=False, load_weight=1, model='resnest50', output_size=(480,640)):
        ...
        self.deconv_layer1 = nn.Sequential(
            nn.Conv2d(in_channels = 2048, out_channels = 512, kernel_size = 3, padding = 1, bias = False),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
        self.deconv_layer2 = nn.Sequential(
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, num_channels=3, train_enc=False, load_weight=1, output_size=(480,640), readout="simple"):
        ...
        self.model = timm.create_model('tf_efficientnet_b0.aa_in1k', pretrained=True) 
...
class EfficientNetB4(nn.Module):
    ...
    def __init__(self, num_channels=3, train_enc=False, load_weight=1, output_size=(480,640), readout="simple"):
        ...
        self.model = timm.create_model('tf_efficientnet_b4.aa_in1k', pretrained=True) 
...
class EfficientNetB7(nn.Module):
    ...
    def __init__(self, num_channels=3, train_enc=False, load_weight=1, output_size=(480,640), readout="simple"):
        ...
        self.model = timm.create_model('tf_efficientnet_b7.aa_in1k', pretrained=True) 
...
class tresnet(nn.Module):
    def __init__(self, num_channels=3, train_enc=False, load_weight=1, pretrained='21k', output_size=(480,640)):
        ...
        self.num_channels = num_channels
        if pretrained == '21k':
            self.resnet = timm.create_model('tresnet_m.miil_in21k', pretrained=True) 
        else: 
        ...
        self.deconv_layer1 = nn.Sequential(
            nn.Conv2d(in_channels = 2048, out_channels = 128, kernel_size = 3, padding = 1, bias = False),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
...
class MobileViTv2(nn.Module):
    ...
    def __init__(self, num_channels=3, train_enc=False, load_weight=1, output_size=(480,640), readout="simple"):
        ...
        self.num_channels = num_channels
        self.model = timm.create_model('mobilevitv2_150.cvnets_in22k_ft_in1k_384', pretrained=True)
        del self.model.head

        self.head = Decoder(channels=[768, 576, 384, 192, 48, 48], act="Swish", output_size=output_size, readout=readout) 
