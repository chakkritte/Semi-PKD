...
def loss_func(pred_map, gt, fixations, args):
    loss_distribution = torch.FloatTensor([0.0]).cuda()
    loss_location = torch.FloatTensor([0.0]).cuda()
    loss_distribution += kldiv(pred_map, gt)
    loss_distribution += (1 - cc(pred_map, gt))
    nss_value = nss(pred_map, fixations)
    loss_location += 1 - ((torch.exp(nss_value) / (1 + torch.exp(nss_value))))
    return loss_distribution + loss_location

def loss_func_self(pred_map, gt, fixations, args):
    loss = torch.FloatTensor([0.0]).cuda()
    loss += kldiv(pred_map, gt)
    loss += (1 - cc(pred_map, gt))
    loss += (1 - similarity(pred_map, gt))
    return loss
...
def cross_entropy_loss(pred, target, weights=0.5, batch_average=False, reduce=True):
...
def cc_score(x, y, weights=2, batch_average=False, reduce=True):
    # x = x.squeeze(1)
    # x = torch.sigmoid(x)
    # y = y.squeeze(1)
